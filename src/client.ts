...
import { V1, V1RetrieveMenusParams, V1RetrievePermissionsParams } from './resources/v1/v1';
...
export interface ClientOptions {
  /**
   * Defaults to process.env['DEMO01_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['DEMO01_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['DEMO01_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Demo01 API. 
 */
export class Demo01 {
  apiKey: string | null;
   ...
   * API Client for interfacing with the Demo01 API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['DEMO01_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['DEMO01_BASE_URL'] ?? http://aipod-backend] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('DEMO01_BASE_URL'),
    apiKey = readEnv('DEMO01_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `http://aipod-backend`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Demo01.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('DEMO01_LOG'), 'process.env[\'DEMO01_LOG\']', this) ?? defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'http://aipod-backend';
  }
  ...
  static Demo01Error = Errors.Demo01Error;
...
export declare namespace Demo01 {
