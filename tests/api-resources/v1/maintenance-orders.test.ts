...
describe('resource maintenanceOrders', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.v1.maintenanceOrders.list({
    maintenanceId: 0,
    orderStatus: 0,
    page: 0,
    podId: 0,
  ...
  test.skip('escalate: required and optional params', async () => {
    const response = await client.v1.maintenanceOrders.escalate(0, { upgradeReason: 'upgradeReason', upgradedMaintenanceId: 'upgradedMaintenanceId' });
  });

  // Mock server tests are disabled
  test.skip('statistics: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.v1.maintenanceOrders.statistics({ maintenanceId: 0, podId: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Demo01.NotFoundError);
  });
});
